...
source_server = {}
target_server = {}
source_sql_server: sqlserver.SqlServerObject = None
target_sql_server: sqlserver.SqlServerObject = None
...
class UPDATE(SYNCSQL):
    def __call__(self, sql):
        ...
        return result, f"update [{table_name}] set {set_clause[:-1]} where id = {result['id']}", table_name
...
def selcet_ip(request):
    ...
    global source_server, target_server, source_sql_server, target_sql_server
    json_data = json.loads(request.body)
    s_db_list = []
    t_db_list = []
    if json_data.get('source') and json_data.get('source').get('ip'):
        try:
            ...
            source_sql_server = sqlserver.SqlServerObject(host=json_data.get('source')['ip'],
...
def selcet_db(request):
    ...
    if json_data.get('target') and json_data.get('target').get('db'):
        try:
            json_data['target'].pop('zd', '')
            target_server = json_data.get('target')
            target_sql_server = sqlserver.SqlServerObject(host=json_data.get('target').get('ip'),
                                                          ssh_host=json_data.get('target').get('ssh_ip'),
...
def table_field_compare(request):
    ...
    global source_server, target_server, source_sql_server, target_sql_server
    source_sql_server, target_sql_server = json_result.get('source_sql_server'), json_result.get('target_sql_server')
    try:
        if not json_result.get('source_sql_server'):
            source_sql_server = sqlserver.SqlServerObject(host=source_server['ip'],
                                                          database=source_server['db'],
                                                          ssh_host=source_server.get('ssh_ip'),
                                                          **source_server)  # 开发服务器
        if not json_result.get('target_sql_server'):
            target_sql_server = sqlserver.SqlServerObject(host=target_server['ip'],
                                                          database=target_server['db'],
                                                          ssh_host=target_server.get('ssh_ip'),
                                                          **target_server)  # 服务器
