...
class MainData:
    ...
    def __init__(self, table_name: Union[str] = 'TAB', action: str = 'ADD', fields_info: Union[dict, list] = []):
        ...
        if action == 'CREATE':
            ...
            for i in fields_info:
                if i['primary_key']:
                    ...
                    info += (i['column_name'] + ' ' + i['data_type'] + '(' + i['max_lenght'] + ')' + ' NULL,') if i[
                                                                                                                      'is_null'] == 'YES' else (
                            i['column_name'] + ' ' + i['data_type'] + '(' + i['max_lenght'] + ')' + ' NOT NULL,')
                elif i['data_type'] == 'date' or i['data_type'] == 'datetime' or i['data_type'] == 'datetime2' or i['data_type'] == 'time':
            ...
            if fields_info['data_type'] == 'text':
            ...
            elif fields_info['max_lenght'] and fields_info['num_max'] is None:
                if action == 'ADD':
                    self.__main_sql = """ALTER TABLE [%s] ADD %s NULL""" % (table_name,
                                                                            fields_info['column_name'] + ' ' +
                                                                            fields_info['data_type'] + '(' + (
                ...
                if action == 'ADD':
                    ...
                    self.__main_sql = """ALTER TABLE [%s] ALTER column %s NULL""" % (table_name, fields_info.get(
                        'column_name') + ' ' + fields_info['data_type'])
            elif fields_info['data_type'] == 'date' or fields_info['data_type'] == 'datetime' or fields_info['data_type'] == 'datetime2' or fields_info[
                'data_type'] == 'time':
                if action == 'ADD':
                    self.__main_sql = """ALTER TABLE [%s] ADD %s NULL""" % (
                        table_name, fields_info.get('column_name') + ' ' + fields_info['data_type'])
...
def baobiao(table_name, source_server, target_server, dict_data, actions):
    print('获取报表', source_server, target_server, dict_data, actions)
    sql = None
    if actions == 'insert':
        fileds = ''
        for k in dict_data.keys():
            fileds += k + ','
        sql = f"""insert into {table_name} {(fileds[:len(fileds) - 1])} SELECT * FROM [{source_server['ip']}].[{source_server['db']}].[dbo].{table_name}"""
    elif actions == 'update':
        fileds = ''
        for k in dict_data.keys():
            fileds += k + '=' + f'b.{k}' + ','
        sql = f"""update {table_name} set {fileds[:len(fileds) - 1]} from {table_name} a, [{source_server['ip']}].[{source_server['db']}].[dbo].{table_name} b where a.id=b.id"""
    ...
    for obj in bd_list_obj:
        if obj.get("table_name") in difference_target_table:
            ...
            data_list: list = [{'column_name': str(obj[0]), 'data_type': obj[1],
                                'max_lenght': (str(obj[2]) if bool(obj[2]) != False else obj[2]), 'is_null': obj[3],
...
def table_field_compare(request):
    ...
    for obj in data_list:
        ...
        # 索引有差异的字段列表
    ceshi = set(source_index_list) - set(target_index_list)
    bd_index_list = [i[2] for i in ceshi]
    data_list[0]['sql_indexes'] = []
    # and j.split(' ')[0].find(',') == -1
    for k in s_helpindex_sql:
        if k[2] in bd_index_list:
            if k[2] == 'id':
                continue
...
def table_data_compare(request):
    ...
    for obj in source_data_list:
        ...
        if len(target_data) == 0:  # insert
            # sql = baobiao(table_name, source_server, target_server, obj, 'update')
            # if target_identity_list:
            #     sql = f"SET IDENTITY_INSERT [{table_name}] ON {sql} SET IDENTITY_INSERT [{table_name}] OFF"
            for k, v in obj.items():
                column += f'{k},'
                null = 'null'
                if v == True:  # 这里存在 为None    所以必须判断 TRUE
                    value += f"""1,"""
                elif v == False:
                    value += f"""0,"""
                elif v == '':
                    ...
                    baobiao_sql = baobiao(table_name, source_server, target_server, obj, 'insert')
                    value = None
                    continue
                else:
                    value += f"""{(v if isinstance(v, int) else "'{}'".format(v)) if v else null},"""
            if value:
                if target_identity_list:
            ...
            for i in source_data_list:
                if i['id'] == obj['id']:
                    i['sql'] = sql if sql else baobiao_sql  # insert
        else:  # update
            for k, v in obj.items():
                if v == True and k != 'id':  # 这里存在 为None    所以必须判断 TRUE
                    temp_sql_str += str(k) + '=' + "1,"
                elif k == 'id':
                    continue
                elif v == False:
                    temp_sql_str += str(k) + '=' + "0,"
                elif v == '':
                    temp_sql_str += str(k) + '=' + "'',"
                elif isinstance(v, datetime.date):
                    temp_sql_str += str(k) + '=' + "'" + v.strftime("%Y-%m-%d") + "'" + ','
                elif isinstance(v, datetime.datetime):
                    temp_sql_str += str(k) + '=' + "'" + v.strftime("%Y-%m-%d %H:%M:%S") + "'" + ','
                elif isinstance(v, datetime.time):
                    temp_sql_str += str(k) + '=' + "'" + v.strftime("%H:%I:%M") + "'" + ','
                elif isinstance(v, bytes):
                    baobiao_sql = baobiao(table_name, source_server, target_server, obj, 'update')
                    temp_sql_str = None
                    continue
                else:
                    temp_sql_str += str(k) + '=' + (
                        "{}".format(v if isinstance(v, int) else f"'{v}'") if v else 'null') + ','
            if temp_sql_str:
                sql = f"update [{table_name}] set {temp_sql_str[:len(column) - 1]} where id = {obj['id']}"
            for i in source_data_list:
                if i['id'] == obj['id']:
                    i['sql'] = sql if sql else baobiao_sql  # update
...
def data_synchronization(request):
    ...
    # sql1 = "insert into [user_test] values (7, '刘德华7', 54, '男', 1);COMMIT"
    # sql2 = "update [user_test] set [id] = <id, int,> , [name] = <name, varchar(255),>"
    try:
        target_sql_server.insert_data(sql)
    except Exception as e:
...
@require_http_methods(['GET'])
def get_cllb_tree(request):
    source_sql_server = sqlserver.SqlServerObject(host="172.17.18.110",
                                                  port=1433,
                                                  user="sa",
                                                  password="CgSqlServerRoot2012",
                                                  database="cgyypt")  # 开发服务器
    sql = "SELECT * FROM jxc_csh_cllb"
    result_set, column_list = source_sql_server.query_data(sql)

    def get_child(source, id_=-1, leval=1):
        sz = []
        for j in source:
            if j.get('sjid', 0) == id_:
                next_leval = leval + 1
                # 循环生成dict
                dict_ = {'id': j.get('id'), 'lbdm': j.get('lbdm'), 'lbmc': j.get('lbmc'), 'sxs': j.get('sxs'),
                         'sjid': j.get('sjid'), 'ccs': j.get('ccs'), 'pym': j.get('pym'), 'bz': j.get('bz'),
                         'ty': j.get('ty'), 'sys': j.get('sys'), 'leval': leval,
                         'child': get_child(source, j.get('id'), leval=next_leval)}
                sz.append(dict_)
        return sz

    alldata = [dict(zip(column_list, row)) for row in result_set]
    return JsonResponse({"data": get_child(alldata)})
...
def get_change_table(request):
    ...
    #global source_sql_server, target_sql_server
    #is_change = False
    #tablename = json.loads(request.body).get('table_name')
    # print(json.loads(request.body).get('table_name'))
    #try:
    #    source_data = source_sql_server.query_data(f"SELECT * FROM [{tablename}]")[0]
    #except:
    #    source_data = set()
    #try:
    #    target_data = target_sql_server.query_data(f"SELECT * FROM [{tablename}]")[0]
    #except:
    #    target_data = set()  # 已知服务器没有这张表报错
    #if len(source_data.difference(target_data)):
    #    is_change = True
    return JsonResponse({'code': 200, 'message': '成功', 'data': True})